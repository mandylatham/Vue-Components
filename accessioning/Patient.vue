            ...
            <div class="form-group">
                ...
                <div class="row">
					...
					<div class="form-group col-lg-4 col-auto">
						...
						<div id="dobField" v-bind:class="['datefield', 'text-nowrap', {'disabled': !this.allowEditSave}]" >
							<input id="month111" name="month" type="text" maxlength="2" placeholder="MM" v-model.number.lazy="computedDobMonth" v-bind:disabled="!this.allowEditSave" />/
							<input id="day" name="day" type="text" maxlength="2" placeholder="DD" v-model.number.lazy="computedDobDay" v-bind:disabled="!this.allowEditSave" /> /
							<input id="year"  name="year" type="text" maxlength="4" placeholder="YYYY" v-model.number.lazy="computedDobYear" v-bind:disabled="!this.allowEditSave" />
						</div>
                ...
                <div class="row">
                    <div class="col-auto" v-if="this.allowEditSave">
                                ...
                                <transition name="collapse-transition">
                                    <div class="row" v-show="!this.detailsCollapsed">
                                        ...
                                        <div v-for="att in getPatientAttributesBySection('Demographics')" class="form-group col-lg-12 col-auto">
                                            <label v-bind:for="att.name+'_'+att.type" class="label-above">{{att.label}}</label>
                                            <div class="input-group">

                                                <GooglePlaces v-if="att.type==='location-general'"
                                                              v-bind:prop_id="att.name+'_'+att.type"
                                                              v-bind:value="currentPatientAttributeValue(patientState.patient, att.name, true)"
                                                ...
                                                <div v-if="att.type==='single-small'" v-bind:id="att.name+'_'+att.type" class="btn-group pl-1" data-toggle="buttons">
                                                    <label v-for="option in att.options"
                                                           v-bind:class="option.id === currentPatientAttributeValue(patientState.patient, att.name, true).id ? 'btn btn- btn-radio active' : 'btn btn-radio'"
                                                        ...
                                                        <input type="radio" autocomplete="off" name="option" v-bind:id="option.id"
                                                               v-bind:checked="option.id === currentPatientAttributeValue(att.name, true).id"
                                                ...
                                                <div v-if="att.type==='multiple-small'" v-bind:id="att.name+'_'+att.type" class="btn-group pl-1" data-toggle="buttons">
                                                    <label v-for="option in att.options" class="btn btn-radio"
                                                           v-bind:class="currentPatientAttributeValue(patientState.patient, att.name, false).find(function(v) {option.id === v.id}) ? 'btn btn-radio active' : 'btn btn-radio'"
                                                           v-on:click="updatePatientAttributeValue(att.name, option, true)">
                                                        <input type="checkbox" autocomplete="off" name="option" v-bind:id="option.id"
                                                               v-bind:checked="currentPatientAttributeValue(patientState.patient, att.name, false).find(function(v) {option.id === v.id})"
                      ...
                      <multiselect v-if="att.type==='single-large'"
                                   ...
                                   v-bind:value="currentPatientAttributeValue(patientState.patient, att.name, true)"
                                   v-bind:style="setMinWidth(att.options)"
                                   v-on:input="updatePatientAttribute">
                      </multiselect>
                      ...
                      <multiselect v-if="att.type==='multiple-large'"
                                   ...
                                   deselect-label=""
                                   select-label=""
                                   v-bind:value="currentPatientAttributeValue(patientState.patient, att.name, false)"
                                   v-bind:style="setMinWidth(att.options)"
                                   v-on:input="updatePatientAttribute">
                      </multiselect>
